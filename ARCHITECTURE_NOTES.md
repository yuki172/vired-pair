...
The implementation targets a _relation detection_ task in electrical plans:
...
applied to the residual stream _before_ it is passed into each sub-layer, and
...
first", but the implementation enforced `i < j` (lower _index_ first). For a
...
| Field          | Detail                                         |
| -------------- | ---------------------------------------------- |
| File           | `models/vired_model.py` → `ViREDModel.predict` |
